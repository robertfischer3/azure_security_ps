...
foreach ($subscription in $subscriptions) {
    ...
    foreach ($vm in $getAllVMInSubscription) {
        ...
        if ($osDisk) {
            ...
            $encryptionTypeOs = Get-ValueOrStringNull -InputObject $disk.EncryptionSettingsCollection.EncryptionSettings
            ...
            $recordOs = [PSCustomObject]@{
                ...
                EncryptionType     = if ($encryptionTypeOs) { "ADE" } else { "None or Other" }                
            }
            Write-Host "Record: " $recordOs
            if ($null -ne $recordOs) {
                $vmDiskCollection += $recordOs
                $total_records += 1

            }
            else {
                Write-Host "Null record issue: " $recordOs
            }
            $recordOs = $null
        }
        ...
        foreach ($dataDisk in $vmDetails.StorageProfile.DataDisks) {
            ...
            if ($disk) {
                ...
                $encryptionType = Get-ValueOrStringNull -InputObject $disk.EncryptionSettingsCollection.EncryptionSettings
                ...
                $recordData = [PSCustomObject]@{
                    ...
                    EncryptionType     = if ($encryptionType -ne "Null") { "ADE" } else { "None or Other" }
                }
                Write-Host "Record: " $recordData
                if ($null -ne $recordData){
                    $vmDiskCollection += $recordData
                    $total_records += 1
                }
                else {
    ...
    Write-Host "Current Record Count: " $total_records

    foreach ($item in $vmDiskCollection) {
        if ($null -eq $item) {
            Write-Host "Null item found"
            $nullCount += 1
        }
        else {
            $verifiedItems += $item
...
Write-Host "Total Record Count: " $total_records
Write-Host "Total Null Count: " $nullCount